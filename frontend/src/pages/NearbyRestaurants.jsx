  ...
  const fetchRestaurants = async () => {
    ...
    try {
      ...
      if (useNearMe && userLocation) {
        ...
        url = `${API_BASE_URL}/openmenu/location?country=MA`;
      } else {
        url = `${API_BASE_URL}/openmenu/location?city=${selectedCity}&country=MA`;
      }
  ...
  const handleMapMove = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/openmenu/nearby?lat=${center.lat}&lng=${center.lng}&radius=${distanceFilter || 5}`
      );
      const data = await response.json();
