...
import {
  ...
  ClipboardList, Mail, Phone, Utensils, Bike, Globe, EyeOff
} from 'lucide-react';
import {
...
const AdminDashboard = () => {
  ...
  const handleSendNotify = async (e) => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/complaints/${selectedNotifyComplaint.id}/notify`,
        notifyForm,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const fetchNotifications = async () => {
    try {
      const { data } = await axios.get(`${API_BASE_URL}/notifications`, { headers:{ Authorization:`Bearer ${token}` } });
  ...
  const fetchInsights = async () => {
    ...
    try {
      const { data } = await axios.get(`${API_BASE_URL}/ai/forecast/insights`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setAiInsights(data);
    } catch (err) {
  ...
  const resetPassword = async () => {
    if (!resetPwModal || !resetPwValue.trim()) return;
    try {
      await put(`users/${resetPwModal.id}/reset-password`, { password: resetPwValue });
      setResetPwModal(null); setResetPwValue('');
      alert('Mot de passe réinitialisé !');
    } catch (err) { alert(err.response?.data?.error || 'Erreur'); }
  };
  const toggleResto  = id     => put(`restaurants/${id}/toggle`).then(fetchRestaurants);
  const verifyResto  = (id, verified) => put(`restaurants/${id}/verify`, { isVerified: verified }).then(fetchRestaurants);
  ...
  const toggleCoupon = id     => axios.put(`${API_BASE_URL}/coupons/${id}/toggle`,{},{headers:{Authorization:`Bearer ${token}`}}).then(fetchCoupons);
  const deleteCoupon = id     => window.confirm('Supprimer?') && axios.delete(`${API_BASE_URL}/coupons/${id}`,{headers:{Authorization:`Bearer ${token}`}}).then(fetchCoupons);
  ...
  return (
            ...
            <motion.div key={tab} initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-8 }} transition={{ duration:.15 }}>
                        ...
                        <tbody>
                          {filter(users,['name','email','role']).map(u => (
                            <tr key={u.id} onMouseEnter={e=>e.currentTarget.style.background='#FAFBFD'} onMouseLeave={e=>e.currentTarget.style.background='#fff'} style={{ transition:'background .15s' }}>
                              ...
                              <TD><Btn onClick={()=>{ setResetPwModal({id:u.id,name:u.name}); setResetPwValue(''); }} variant="ghost" style={{fontSize:12}}>🔑 MDP</Btn></TD>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              {tab === 'restaurants' && (
                <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                  ...
                  {loading ? <Spin /> : (
                  <>
                  {/* ── Unverified / Pending restaurants ── */}
                  {restaurants.filter(r => !r.isVerified).length > 0 && (
                    <div style={{ background:'#FFFBEB', border:'1.5px solid #FCD34D', borderRadius:16, padding:20 }}>
                      <p style={{ margin:'0 0 14px', fontWeight:800, fontSize:14, color:'#92400E', display:'flex', alignItems:'center', gap:6 }}>
                        ⏳ En attente de validation ({restaurants.filter(r => !r.isVerified).length})
                      </p>
                      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:12 }}>
                        {restaurants.filter(r => !r.isVerified).map(r => (
                          <div key={r.id} style={{ background:'#fff', borderRadius:12, overflow:'hidden', boxShadow:'0 2px 8px rgba(0,0,0,0.06)', border:'1.5px solid #FCD34D' }}>
                            <div style={{ position:'relative' }}>
                              {r.image_url
                                ? <img src={r.image_url} alt={r.name} style={{ width:'100%', height:120, objectFit:'cover' }} />
                                : <div style={{ width:'100%', height:120, background:'linear-gradient(135deg,#FEF3C7,#FDE68A)', display:'flex', alignItems:'center', justifyContent:'center' }}><Store size={32} color="#F59E0B" /></div>
                              }
                              <div style={{ position:'absolute', top:8, right:8 }}>
                                <Badge label="Non validé" color="#92400E" bg="#FEF3C7" />
                              </div>
                            </div>
                            <div style={{ padding:14 }}>
                              <p style={{ margin:'0 0 3px', fontWeight:800, fontSize:14, color:C.text }}>{r.name}</p>
                              <p style={{ margin:'0 0 2px', color:C.sub, fontSize:11, display:'flex', alignItems:'center', gap:3 }}><MapPin size={10}/>{r.city} · {r.cuisine}</p>
                              <p style={{ margin:'0 0 10px', color:C.muted, fontSize:11, display:'flex', alignItems:'center', gap:4 }}><User size={10}/> {r.owner_name}</p>
                              <div style={{ display:'flex', gap:8 }}>
                                <Btn onClick={(e)=>{ e.stopPropagation(); verifyResto(r.id, true); }} variant="success" style={{ flex:1, justifyContent:'center', fontSize:12 }}>✓ Valider</Btn>
                      ...
                      <motion.div key={r.id} whileHover={{ y:-4, boxShadow:'0 16px 32px rgba(0,0,0,0.10)' }} transition={{ type:'spring', stiffness:300, damping:22 }}
                        ...
                        <div style={{ padding:18 }}>
                          ...
                          <div style={{ display:'flex', gap:8 }}>
                            ...
                            <Btn onClick={(e)=>{ e.stopPropagation(); if(window.confirm('Retirer ce restaurant du site?')) verifyResto(r.id, false); }} variant="danger" style={{ padding:'7px 10px' }} title="Retirer du site"><EyeOff size={13}/></Btn>
                            <Btn onClick={(e)=>{ e.stopPropagation(); deleteResto(r.id); }} variant="danger" style={{ padding:'7px 10px' }}><Trash2 size={13}/></Btn>
                          </div>
                        </div>
                      </motion.div>
                    ))}
                    {!filter(restaurants.filter(r => r.isVerified),['name','city','cuisine','owner_name']).length && <p style={{ color:C.muted }}>Aucun résultat</p>}
                  </div>
                  </>
                  )}
                </div>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                  ...
                  <div style={{ background:'#fff', borderRadius:16, padding:'22px 24px', boxShadow:'0 2px 16px rgba(0,0,0,0.05)' }}>
                    ...
                    <form onSubmit={async e => {
                      e.preventDefault();
                      try { await axios.post(`${API_BASE_URL}/coupons`, couponForm, { headers:{ Authorization:`Bearer ${token}` } }); setCouponForm({ code:'', discount_type:'percentage', discount_value:'', min_order:'', max_uses:'' }); fetchCoupons(); }
    ...
    {resetPwModal && (
      <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:5000, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}
        ...
        <div style={{ background:'#fff', borderRadius:20, padding:'28px 32px', maxWidth:400, width:'100%', boxShadow:'0 24px 64px rgba(0,0,0,0.2)', fontFamily:'Outfit,sans-serif' }}
          onClick={e => e.stopPropagation()}>
          <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:20 }}>
            ...
            <div>
              <p style={{ margin:0, fontWeight:800, fontSize:16, color:C.text }}>Réinitialiser le mot de passe</p>
              <p style={{ margin:'2px 0 0', fontSize:12, color:C.muted }}>{resetPwModal.name}</p>
            </div>
          </div>
          <input
            type="password"
            ...
            style={{ width:'100%', padding:'12px 14px', borderRadius:11, border:`1.5px solid ${C.border}`, fontSize:13, fontFamily:'Outfit,sans-serif', outline:'none', boxSizing:'border-box', color:C.text }}
          ...
          <div style={{ display:'flex', gap:10, marginTop:16 }}>
            ...
            <button onClick={resetPassword}
              style={{ flex:1, padding:11, borderRadius:10, border:'none', background:'#3B82F6', color:'#fff', fontWeight:700, fontSize:13, cursor:'pointer', fontFamily:'Outfit,sans-serif', boxShadow:'0 4px 12px rgba(59,130,246,0.35)' }}>
              Confirmer
            </button>
          </div>
        </div>
